...
class CommunicateThread(threading.Thread):
            ...
            if not self.frameQueue.empty():
                ...
                if frame_num == frame_to_go:
                    ...
                    if curr_address in self.status['path']:
                        # pdb.set_trace()
                        self.stopAddress(frame, curr_address)
                    elif curr_address == 'stop':
                        self.stopSTOP(frame)
    ...
    def stopAddress(self, curr_frame, location):
        move("STOP")
        self.status['mode'] = 'stop'
        self.status['location'] = location
...
if __name__=='__main__':
    ...
    thread3 = WebSocketThread(status=status, uri="ws://172.26.226.86:3000/robot")
    thread1.daemon = True
    thread2.daemon = True
    thread3.daemon = True
