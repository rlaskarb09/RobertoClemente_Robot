...
FRAME_INTERVAL = []
ADDRESS = ['201', '202', '203', '103', '102', '101', 'STOP']
...
class CommunicateThread(threading.Thread):
    ...
    def linetrack(self):
        ...
        while True:
            if not self.frameQueue.empty():
                ...
                if self.status['command'] == 'maintenance':
                    self.stopMaintanence(frame, curr_address, curr_address)
                elif frame_num == frame_to_go:
        ...
        #             main_flag = 1
        #             self.stop(frame, curr_address, curr_address)
        #             frame_num -= 1
        #         elif self.status['command'] =='move':
        #             self.status['mode'] = 'move'
        #             main_flag = 0
        #             move("FORWARD")
        #             last_shift, last_angle = self.getAction(frame, last_shift, last_angle, self.showFrame, start)
        ...
        #             next_address = ADDRESS[idx]
        #             frame_to_go = FRAME_INTERVAL[next_address]
        #             self.stop(1, frame, curr_address, next_address)
        #             frame_num =0
        #         else:
        #             move("FORWARD")
        #             last_shift, last_angle = self.getAction(frame, last_shift, last_angle, self.showFrame, start)
    ...
    def stopMaintanence(self, curr_frame, location, path):
        ...
        while True:
            if self.status['command'] == 'move':
                ...
                self.frameQueue.put(next_frame)
                break
            else:
                continue
